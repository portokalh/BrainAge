...
# -*- coding: utf-8 -*-
"""
GAUDI Training & Visualization – Wasserstein Graphs
---------------------------------------------------
...
def attach_metadata(graphs, meta, numeric_cols):
    ...
    if len(graphs) != len(meta):
        print(f"⚠️ Mismatch: {len(graphs)} graphs vs {len(meta)} metadata rows.")
...
if __name__ == "__main__":
    ...
    print(f"[Joint] Latent embedding saved → latent_final_joint_{Z_PREFIX}.npy")

    meta_joint = visualize_embeddings(z_joint, f"Joint_{Z_PREFIX}", meta, joint_dir)
    print("\n🎯 Wasserstein GAUDI training + visualization complete.")
