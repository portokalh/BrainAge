...
- Saves CSVs of all + healthy (to your requested folder)
...
# Metadata can be CSV or XLSX; set one path and (if XLSX) a sheet:
HABS_META_PATH = "/home/bas/Desktop/MyData/harmonization/HABS/metadata/HABS_metadata.xlsx"
HABS_META_SHEET = 0  # or a sheet name; ignored for CSV
...
def load_and_clean_metric_to_mri_exam(
    ...
    """
    Read studywide stats (tsv), select subject columns, transpose to subjects x ROIs,
    coerce numeric, and index by MRI_Exam_fixed-style keys (e.g., 'H4369_y0').
    """
...
# Save CSVs to your requested folder
...
print(f"Saved: {os.path.join(OUTDIR, 'HABS_with_risk.csv')}")
print(f"Saved: {os.path.join(OUTDIR, 'HABS_healthy_controls.csv')}")
...
print("Removal summary:", {"total_removed": int(len(removed_df)), "diabetes": int(n_diab), "dementia": int(n_dem), "both": int(n_both)})
...
# Save enriched healthy metadata to your folder
df_metadata_PCA_healthy_withConnectome.to_csv(
    os.path.join(OUTDIR, "HABS_healthy_metadata_with_placeholders.csv"),
    index=False
)
print(f"Saved: {os.path.join(OUTDIR, 'HABS_healthy_metadata_with_placeholders.csv')}")
...
torch.save(multimodal_features_dict,
           os.path.join(RESULTS_DIR, "multimodal_features_dict.pt"))

# Save normalized multimodal features
torch.save(normalized_node_features_dict,
           os.path.join(RESULTS_DIR, "normalized_node_features_dict.pt"))

# Save demographics mapping
torch.save(subject_to_demographic_tensor,
           os.path.join(RESULTS_DIR, "subject_to_demographic_tensor.pt"))

print("\n✅ Saved tensors to:", RESULTS_DIR)
print(" - multimodal_features_dict.pt")
print(" - normalized_node_features_dict.pt")
print(" - subject_to_demographic_tensor.pt")
